...
void ViewOrganismDialog::DDX_Cell(CDataExchange* pDX)
{
	...
	DDX_CommaValue(pDX, IDC_VO_MOOD, cell->marker);

	Grid_Get(organism->universe,cell->x,cell->y,&ugrid);
	sprintf(buf, "(%d, %d)", ugrid.energy_free/ei, cell->energy_stored/ei);
	str = buf;
	DDX_Text(pDX, IDC_VO_MESSAGE , str);

	sprintf(buf, "(%d, %d, %d, %d)", cell->weigh_energy[0]/ei,cell->weigh_energy[1]/ei,cell->weigh_energy[2]/ei,cell->weigh_energy[3]/ei);
	str = buf;
	DDX_Text(pDX, IDC_VO_CELLIDX , str);

	sprintf(buf, "(%d, %d, %d, %d)", cell->weigh_secretin[0]/s_,cell->weigh_secretin[1]/s_,cell->weigh_secretin[2]/s_,cell->weigh_secretin[3]/s_);
	str = buf;
	DDX_Text(pDX, IDC_VO_LOCATION, str);

	vct=cell->kfm->vec_s.data[cell->kfm->vec_s.pos];
	sprintf(buf, "(%I64d, %I64d, %I64d, %I64d)", vct.vec_num[0],vct.vec_num[1],vct.vec_num[2],vct.vec_num[3]);
	str = buf;
	DDX_Text(pDX, IDC_VO_CB, str);

	DDX_CommaValue(pDX,IDC_VO_PC,cell->kfm->Rc);
	/*
	idx = 0;
...
void ViewOrganismDialog::DDX_machine_registers(CDataExchange* pDX)
{
	...
	sprintf(buf, "%I64d", kfm->Rn[0]);
	str = buf;
	DDX_Text(pDX, IDC_VO_R0, str);

	sprintf(buf, "%I64d", kfm->Rn[1]);
	str = buf;
	DDX_Text(pDX, IDC_VO_R1, str);

	sprintf(buf, "%I64d", kfm->Rn[2]);
	str = buf;
	DDX_Text(pDX, IDC_VO_R2, str);

	sprintf(buf, "%I64d", kfm->Rn[3]);
	str = buf;
	DDX_Text(pDX, IDC_VO_R3, str);

	sprintf(buf, "%I64d", kfm->Rn[4]);
	str = buf;
	DDX_Text(pDX, IDC_VO_R4, str);

	sprintf(buf, "%I64d", kfm->Rn[5]);
	str = buf;
	DDX_Text(pDX, IDC_VO_R5, str);

	sprintf(buf, "%I64d", kfm->Rn[6]);
	str = buf;
	DDX_Text(pDX, IDC_VO_R6, str);

	sprintf(buf, "%I64d", kfm->Rn[7]);
	str = buf;
	DDX_Text(pDX, IDC_VO_R7, str);
/*   
	sprintf(buf, "%I64d", kfm->Rn[8]);
	str = buf;
	DDX_Text(pDX, IDC_VO_R8, str);

	sprintf(buf, "%I64d", kfm->Rn[9]);
	str = buf;
...
void ViewOrganismDialog::DDX_data_stack(CDataExchange* pDX)
{
	...
	for(i=0;i<10;i++) {
		...
		sprintf(buf, "%I64d", kfm->num_s.data[n]);
...
void ViewOrganismDialog::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_CommaValue(pDX, IDC_VO_ENERGY, organism->direction);
