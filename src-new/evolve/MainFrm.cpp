...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// set up status bar
	//
	m_wndStatusBar.SetPaneInfo(0, 1, SBPS_NORMAL, 110);
	m_wndStatusBar.SetPaneInfo(1, 2, SBPS_NORMAL, 140);
	...
	m_wndStatusBar.SetPaneInfo(5, 5, SBPS_NORMAL, 200);
	m_wndStatusBar.SetPaneInfo(6, 6, SBPS_NORMAL, 120);
...
void CMainFrame::set_status_cursor2(int x, int y, int energy,int secretin)
{
	CMDIChildWnd *child;
	char buf[100];
	...
	if( child ) {
		...
		m_wndStatusBar.SetPaneText(5, buf);
		sprintf(buf, "secretin: %d",secretin);
...
void CMainFrame::set_status_cursor3(int x, int y, int energy,int stored,int secretin)
{
	CMDIChildWnd *child;
	char buf[100];
	...
	if( child ) {
		sprintf(buf, "(%d, %d)  energy: %d; %d", x, y, energy,stored);
		m_wndStatusBar.SetPaneText(5, buf);
		sprintf(buf, "secretin: %d",secretin);
		m_wndStatusBar.SetPaneText(6, buf);
	} else {
