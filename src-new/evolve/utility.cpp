...
// This is a safety assurance to many functions
static void creat_barrier_rectangle(UNIVERSE *u)
{
	int x,y;
	for(x=0;x<u->width;x++){
		Universe_SetBarrier(u, x, 0);
		Universe_SetBarrier(u, x, u->height-1);
	}
	for(y=0;y<u->height;y++){
		Universe_SetBarrier(u, 0, y);
		Universe_SetBarrier(u, u->width-1, y);
	}
}
...
static void create_population(UNIVERSE *u, int population, int total_energy, ORGANISM *o)
{
	...
	for(i=0; i < population; i++) {
		...
		if( i == 0 ) {
			no = o;
			no->cells->energy_stored = energy_per_organism + energy_remainder;
		} else {
			no = Universe_CopyOrganism(u);
			Universe_ClearSelectedOrganism(u);
			no->cells->energy_stored = energy_per_organism;
		}
