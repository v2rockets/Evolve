...
void CevolveView::update_status_cursor(CPoint &point)
{
	...
	if( xi >= 0 && xi < u->width && yi >= 0 && yi < u->height ) {
		...
		if( gt == GT_SPORE ) {
			show_energy = 2;
			stored = ugrid.u.spore->energy;
		} else if( gt == GT_ORGANIC ) {
			show_energy = 2;
			stored = ugrid.u.energy/ei;
		} else if( gt == GT_CELL ) {
			show_energy = 2;
			stored = ugrid.u.cell->energy_stored/ei;
		} else
			show_energy=1;

		energy=ugrid.energy_free/ei;
		secretin=ugrid.secretin/s_;
	}


	switch (show_energy ) {
		case 1:
			mainFrame->set_status_cursor2(xi, yi, energy,secretin);
			break;
		case 2:
			mainFrame->set_status_cursor3(xi, yi, energy,stored,secretin);
			break;
		default:
...
void CevolveView::OnRButtonDown_Energy(UINT nFlags, CPoint point)
{
	...
	if( gt == GT_CELL ) {
		organism = ugrid.u.cell->organism;
		energy1 =  ugrid.u.cell->energy_stored;
	...
	if( nRet == IDOK ) {
		...
		if( energy2 > 0 && energy2 != energy1 ) {
			if( gt == GT_CELL ) {
				organism->cells->energy_stored = energy2; MARK;
